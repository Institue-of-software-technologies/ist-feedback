...
// Define types for the session data and the formatted chart data
interface SessionData {
  loginTime: string;   // assuming loginTime is a string in ISO format
  duration: number;    // duration in seconds
  formattedDuration: string;  // formatted duration like "5m 30s"
}

const AnalysisPage = () => {
  const [usageData, setUsageData] = useState<SessionData[]>([]);  // Store session data here
        ...
        <LineChart data={chartData} margin={{ top: 20, right: 30, left: 50, bottom: 50 }}>
          ...
          <Tooltip
            formatter={(value: number, name: string, props: Payload<number, string>) => {
              // Check if payload has formattedDuration before using it
              const formattedDuration = props.payload?.formattedDuration || `${Math.floor(value / 60)}m ${value % 60}s`;
              return [`${formattedDuration}`, name];  // Use the formatted duration in tooltip
            }}
          />
          <Legend />
